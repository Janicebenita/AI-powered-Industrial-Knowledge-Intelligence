...
export const VERIFIER_CONTRACT = 'industrial-evidence-verification/v1';
...
export type Decision = {index:number;claim_id:string;original_claim:Claim;released_claim?:Claim;outcome:'full'|'narrowed'|'rejected';reason:string;verifier_verdict?:string;verifier_reason?:string;excerpts:ExcerptCheck[]};
...
export function parseVerifierResponse(raw:unknown,submitted:AssignedClaim[]):VerifierResponse {
  ...
  if([v.supported_claim_count,v.partially_supported_claim_count,v.unsupported_claim_count].some((n,i)=>!Number.isInteger(n)||n!==counts[i]))throw Error('verifier count mismatch');
  const overall=counts[0]===submitted.length?'verified':counts[2]===submitted.length?'unverified':'partially_verified';
  if(v.overall_status!==overall)throw Error('verifier overall status mismatch');
  return v;
}
...
export function validateVerdicts(submitted:AssignedClaim[],raw:unknown,evidence:Evidence[]):{decisions:Decision[];entries:number;contract_error?:string} {
  ...
  const decisions=submitted.map(item=>{
    ...
    return {index:item.index,claim_id:item.claim_id,original_claim:item.claim,...(!reason?{released_claim:candidate}:{}),outcome:reason?'rejected':v.verdict==='partially_supported'?'narrowed':'full',reason:reason||'validated',verifier_verdict:v.verdict,verifier_reason:v.reason,excerpts} as Decision;
  });
  return {decisions,entries:response.claim_verdicts.length};
}
...
export async function validateGroundedClaims(value:unknown,evidence:Evidence[],review:(input:Record<string,unknown>)=>Promise<unknown>) {
  ...
  let entries=0,contract_error:string|undefined;
  if(pending.length) {
