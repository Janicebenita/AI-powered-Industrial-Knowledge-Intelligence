...
export function resultFor(execution:Execution) {
  ...
  if(/maintenance.*history|history.*maintenance/i.test(execution.question))claims.sort((a,b)=>(a.text.match(/\b\d{4}-\d{2}-\d{2}\b/)?.[0]||'9999').localeCompare(b.text.match(/\b\d{4}-\d{2}-\d{2}\b/)?.[0]||'9999'));
