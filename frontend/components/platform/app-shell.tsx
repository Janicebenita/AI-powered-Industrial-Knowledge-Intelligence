...
const navGroups = [
  { label: "Command", items: ["/platform/dashboard", "/platform/copilot", "/platform/documents", "/platform/graph", "/platform/entities"] },
  { label: "Operations", items: ["/platform/assets", "/platform/maintenance", "/platform/rca", "/platform/compliance", "/platform/lessons"] },
  ...
  return (
    <div className="relative min-h-screen">
      <div className="pointer-events-none fixed inset-0 opacity-80">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(59,130,246,0.16),transparent_42%)]" />
      </div>
      <div className={cn("relative grid min-h-screen transition-[grid-template-columns] duration-300", collapsed ? "lg:grid-cols-[92px_minmax(0,1fr)]" : "lg:grid-cols-[304px_minmax(0,1fr)]")}>
        <aside className="sticky top-0 z-20 hidden h-screen border-r border-white/10 bg-[#050816]/86 p-4 backdrop-blur-2xl lg:block">
          <div className="mb-5 flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-3">
            <div className="grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br from-blue-500 to-cyan-400 font-black shadow-[0_0_36px_rgba(59,130,246,0.45)]">IB</div>
            {!collapsed ? <div><strong className="text-sm">Industrial Brain AI</strong><p className="text-xs text-slate-400">Unified Operations Intelligence</p></div> : null}
          </div>
                ...
                <div key={group.label}>
                  ...
                  <div className="grid gap-1">
                    {items.map(({ label, href, icon: Icon }) => {
                      ...
                      return (
                        <Link key={href} href={href} aria-current={active ? "page" : undefined} className={cn("group flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm transition", active ? "bg-blue-500 text-white shadow-[0_0_30px_rgba(59,130,246,0.28)]" : "text-slate-400 hover:bg-white/[0.07] hover:text-white")}>
...
function Topbar() {
  return (
    <header className="sticky top-0 z-10 border-b border-white/10 bg-[#050816]/76 px-4 py-3 backdrop-blur-2xl md:px-6 lg:px-8">
      <div className="mx-auto flex max-w-[1580px] items-center gap-3">
        <Link href="/platform/dashboard" className="flex items-center gap-2 lg:hidden">
          <span className="grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-blue-500 to-cyan-400 text-sm font-black">IB</span>
          <span className="hidden text-sm font-bold text-white sm:inline">Industrial Brain AI</span>
        </Link>
        <CommandSearch />
        ...
        <div className="hidden items-center gap-2 rounded-xl border border-emerald-400/25 bg-emerald-400/10 px-3 py-2 text-sm font-semibold text-emerald-100 sm:flex"><Sparkles size={15} /> AI online</div>
        <button className="hidden items-center gap-2 rounded-xl border border-cyan-400/25 bg-cyan-400/10 px-3 py-2 text-sm font-semibold text-cyan-100 sm:flex"><Shield size={15} /> Plant A</button>
    ...
    <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-white/10 bg-[#050816]/94 px-2 py-2 backdrop-blur-2xl lg:hidden">
      <div className="mx-auto grid max-w-xl grid-cols-5 gap-1">
        {mobileNav.map(({ label, href, icon: Icon }) => {
          const active = pathname === href;
          return (
            <Link key={href} href={href} aria-current={active ? "page" : undefined} className={cn("grid min-h-14 place-items-center rounded-xl px-1 text-center text-[11px] transition", active ? "bg-blue-500 text-white" : "text-slate-400 hover:bg-white/[0.07] hover:text-white")}>
              <Icon size={18} />
